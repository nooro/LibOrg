...
public class TakeBookPanel {
	...
	public static void setUp(JFrame window, DBConnector dbConnector) {
		...
		btn_take = new Button("TAKE", Resources.Fonts.LibraryOrganizer.panelButtons, Resources.Colors.white, Resources.Colors.orange);
		btn_take.getJButton().setSize((int) (btn_take.getJButton().getPreferredSize().width * 1.5), (int)(btn_take.getJButton().getPreferredSize().height*1.5));
		btn_take.getJButton().setLocation((int)(panel.getJPanel().getWidth()/2 - btn_take.getSize().width/2), 
										(int)( (tf_bookID.getJTextField().getBounds().y + tf_bookID.getJTextField().getHeight()) + panel.getJPanel().getHeight()*0.02));
	    panel.getJPanel().add(btn_take.getJButton());
	...
	private static void showErrorMessage(String message) {
	    ...
	    errorMessage.setBounds(panel.getJPanel().getWidth()/2 - (int)errorMessageSize.getWidth()/2, 
	    						(int) ( (btn_take.getJButton().getBounds().y + btn_take.getSize().height) + panel.getJPanel().getHeight()*0.02 ), 
	    						errorMessageSize.width, errorMessageSize.height);
	    errorMessage.setVisible(true);
	}
	
	public static void handleEvents() {
		btn_take.getJButton().addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				...
				try {
					BooksOperator.takeBook(db, tf_username.getJTextField().getText(), Integer.parseInt(tf_bookID.getJTextField().getText()));
	...
	public static void setUpEffects() {
		btn_take.getJButton().addMouseListener(new MouseAdapter() {
			@Override
			public void mouseEntered(MouseEvent e) {
				btn_take.getJButton().setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
			} 
		});
